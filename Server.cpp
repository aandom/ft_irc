...
void Server::init_server_address() {
    ...
    if (getaddrinfo(NULL, intToStr(this->port).c_str(), &hints, &this->address) != 0)
        init_error("\033[31m getaddrinfo() failed \033[30m");
}
...
void Server::create_socket() {
    ...
    if (this->socket_fd < 0)
        init_error("\033[31m socket() failed \033[30m");
}
...
void Server::close_connection(int i) {
	std::cout << "Closing connection with fd: " << this->fds[i].fd << "\033[30m" << std::endl;
...
void Server::accept_client () {
	...
	while (true)
	{
		...
		std::cout << "\033[32m Client IP: " << inet_ntoa(new_client_addr.sin_addr) << "\033[30m" << std::endl;
		std::cout << "\033[32m Client fd: " << this->new_sd << "\033[30m" << std::endl;
...
void Server::read_client (int i) {
	...
	while (TRUE)
	{
		...
		if (this->rc < 0) {
			if (errno != EWOULDBLOCK) {
				perror("\033[31m recv() failed \033[30m");
	...
	std::cout << "\033[32m Received " << str.length() << " bytes in the below string \033[30m" << std::endl << str;
